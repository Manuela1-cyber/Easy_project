...
import { CalendarDays, Droplets, Home, Mail, Phone, Receipt, TriangleAlert } from "lucide-react";
import { Appartement, Facture, Locataire, Propriete, getAppartements, getFactures, getLocataires, getProprietes } from "../../../lib/api/facture";
...
export default function EspaceLocatairePage() {
  ...
  useEffect(() => {
    const loadPortalData = async () => {
      ...
      try {
        const [facturesRes, locatairesRes, appartementsRes, proprietesRes] = await Promise.all([
          getFactures(),
          getLocataires(),
          getAppartements(),
          getProprietes(),
  ...
  const appartement = useMemo(
    () => appartements.find((item) => item.locataireId === locataireId),
    [appartements, locataireId]
  );
  ...
  const locataireFactures = useMemo(() => {
    return factures
      .filter((facture) => facture.locataireId === locataireId)
      .sort((a, b) => new Date(b.issuedAt).getTime() - new Date(a.issuedAt).getTime());
  }, [factures, locataireId]);
          ...
          <div className="grid gap-8 px-6 py-8 sm:px-8 lg:grid-cols-[1.4fr_0.9fr] lg:px-10">
            <div>
              ...
              <h1 className="mt-3 text-3xl font-semibold tracking-tight text-gray-900 sm:text-4xl">
                Bonjour {locataire.name}
              </h1>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-gray-500 sm:text-base">
                Retrouvez ici l'historique complet de vos factures d'eau, leur statut et
                les details de consommation de votre logement.
              </p>
